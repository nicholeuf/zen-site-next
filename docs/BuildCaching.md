...
Cache hit/miss statistics are automatically captured as build artifacts (ci-cache-status.json) in GitHub Actions for each workflow run. The JSON file is generated by the reusable script `scripts/gen-ci-cache-status.sh`, which is invoked by all relevant workflows for consistent data capture. These artifacts can be downloaded from the Actions UI or via the GitHub CLI for future analysis.
# Build caching
